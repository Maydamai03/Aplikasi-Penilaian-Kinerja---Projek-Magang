...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="employee-header m-0">
        <img src="{{ $karyawan->foto_profil ? Storage::url('karyawan/' . $karyawan->foto_profil) : 'https://via.placeholder.com/60' }}" alt="Foto">
            ...
            <table class="table mt-4">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        {{-- DIUBAH: Menampilkan bobot yang dihitung otomatis --}}
                        <td>{{ round(($job->durasi_waktu / 480) * 100) }}%</td>
                        <td>{{ $job->durasi_waktu }} menit</td>
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('job.edit', $job->id) }}" class="btn-table btn-edit">Edit</a>
                                <button type="button" class="btn-table btn-delete" onclick="deleteConfirmation({{ $job->id }})">Hapus</button>
                                <form id="delete-form-{{ $job->id }}" action="{{ route('job.destroy', $job->id) }}" method="POST" style="display: none;">@csrf @method('DELETE')</form>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="text-center p-4">Belum ada job untuk shift siang.</td></tr> {{-- DIUBAH --}}
                    @endforelse
                </tbody>
            </table>
            <br>
            <div class="d-flex justify-content-end mt-4">
                <a href="{{ route('penilaian.create', ['karyawan' => $karyawan->id, 'shift' => 'Siang']) }}" class="btn-penilaian">Penilaian Kinerja Shift Siang</a>
            </div>
        </div>
        ...
        <div id="malam" class="tab-panel">
            ...
            <table class="table mt-4">
                ...
                <tbody>
                    ...
                    @empty
                    <tr><td colspan="5" class="text-center p-4">Belum ada job untuk shift malam.</td></tr> {{-- DIUBAH --}}
                    @endforelse
                </tbody>
            </table>
            <br>
            <div class="d-flex justify-content-end mt-4">
                <a href="{{ route('penilaian.create', ['karyawan' => $karyawan->id, 'shift' => 'Malam']) }}" class="btn-penilaian">Penilaian Kinerja Shift Malam</a>
            </div>
        </div>
    </div>
</div>
